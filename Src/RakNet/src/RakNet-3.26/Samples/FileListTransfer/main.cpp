...
public:
		...
		OnFileStruct *onFileStruct)
	{
		...
		FILE *fp = fopen(fileCopy, "wb");
		fwrite(onFileStruct->fileData, onFileStruct->finalDataLength, 1, fp);
		fclose(fp);

		// Make sure it worked
		unsigned int hash1 = SuperFastHashFile(file);
		unsigned int hash2 = SuperFastHashFile(fileCopy);
...
void main(void)
{
	...
	fileList.AddFile(file, 0, fileLength, fileLength, FileListNodeContext(0,0), true);
